...
export class OrgChart {
  constructor() {
    ...
    const attrs = {
      ...
      expandLevel: 7,
      compact: false,
      highlightColor: "#95D162",
      highlightBackgroundColor: "#F4FAEF",
      rootMargin: 40,
        ...
        return `<defs>
                    ${visibleConnections
                      .map((conn) => {
                       ...
                       <path transform="translate(0)" d='M0,0 V4 L2,2 Z' fill='#95D162' />
                       </marker>
                    `;
                      })
                      .join("")}
      ...
      linkUpdate: function (d, i, arr) {
        d3.select(this)
          .attr("stroke", (d) =>
            d.data.parentId !== "root" && d.data._upToTheRootHighlighted
              ? attrs.highlightColor
              : "lightgray"
          )
          .attr("stroke-width", (d) =>
            d.data.parentId !== "root" && d.data._upToTheRootHighlighted ? 5 : 2
          );
      ...
      nodeUpdate: function (d, i, arr) {
        d3.select(this)
          ...
          .style("border-right", (d) =>
            d.data._highlighted ? `1px solid ${attrs.highlightColor}` : ""
          )
          ...
          .style("bottom-left-border-radius", (d) => "6px")
          .style("bottom-right-border-radius", (d) => "6px")
          .style("background-color", (d) =>
            d.data._highlighted ? attrs.highlightBackgroundColor : "#fff"
          );
      },
  ...
  fullscreen(elem) {
    ...
    d3.select(document).on("fullscreenchange." + attrs.id, (d) => {
      const fsElement =
  ...
  exportImg({ full = false, scale = 1, onLoad = (d) => d, save = true } = {}) {
    const that = this;
    const attrs = this.getChartState();
  ...
  exportSvg() {
    ...
    this.downloadImage({ node: svg.node(), scale: 1, isSvg: true });
    return this;
  }
  ...
  downloadImage({
    ...
    image.onload = function () {
      ...
      if (save) {
        ...
        pdf.setProperties({ title: "Org Chart" });
        pdf.viewerPreferences({ DisplayDocTitle: true });
        pdf.text("Org Chart", 15, 22);
        pdf.addImage(dt, "PNG", 0, 40, width, pdfHeight);
